...
//Function to calculate required caloric intake
// function Nutrition_({ userInfo }) {
...
//   const PROTEIN_GOAL = Math.round(weight * 1.7);
//   const FAT_GOAL     = Math.round((CALORIE_GOAL * 0.27) / 9);
//   const CARB_GOAL    = Math.round((CALORIE_GOAL * 0.5) / 4);
//   const FIBRE_GOAL   = sex === "Male" ? 35 : sex === "Female" ? 25 : 30;

//   const totalCals = meals.reduce((sum, m) => sum + m.calories, 0);
//   const progress  = Math.min(totalCals / CALORIE_GOAL, 1);
//   const protein   = Math.round((totalCals * 0.30) / 4);
//   const carbs     = Math.round((totalCals * 0.50) / 4);
//   const fat       = Math.round((totalCals * 0.27) / 9);
//   const fibre     = Math.round(totalCals * 0.012);

//   function addMeal() {
//     if (!mealTitle || !mealCals) return;
//     setMeals(prev => [...prev, { title: mealTitle, calories: Number(mealCals) }]);
//     setMealTitle("");
//     setMealCals("");
//   }
...
//   const suggestions = {
//     Breakfast: [
//       { name: "Scrambled Eggs",    calories: 220, protein: 18, carbs: 2,  fat: 14, recipe: "Whisk 3 eggs. Cook on low heat, stir gently." },
//       { name: "Oatmeal & Berries", calories: 310, protein: 9,  carbs: 54, fat: 6,  recipe: "Cook oats in water 5 min. Top with freeze-dried berries." },
//     ],
//     Lunch: [
//       { name: "Chicken Wrap",      calories: 420, protein: 35, carbs: 38, fat: 10, recipe: "Grill chicken, wrap in tortilla with lettuce and hummus." },
//       { name: "Greek Salad",       calories: 210, protein: 8,  carbs: 14, fat: 14, recipe: "Tomato, cucumber, olives, feta, olive oil, oregano." },
//     ],
//     Dinner: [
//       { name: "Salmon & Rice",     calories: 520, protein: 42, carbs: 48, fat: 12, recipe: "Bake salmon 180°C 15 min. Serve with steamed rice." },
//       { name: "Pasta Bolognese",   calories: 610, protein: 30, carbs: 72, fat: 18, recipe: "Brown mince, add tomato sauce, serve over pasta." },
//     ],
//   };
...
//             <span className="gauge-current">{totalCals}</span>
//             <span className="gauge-goal">/ {CALORIE_GOAL} cal</span>
//           </div>
//         </div>

//         <div className="macro-targets">
//           <div className="macro-target-row"><span style={{ color: "#FC8181" }}>⊙ Protein</span><span>{protein}g / {PROTEIN_GOAL}g</span></div>
//           <div className="macro-target-row"><span style={{ color: "#9F7AEA" }}>⊙ Carbs</span><span>{carbs}g / {CARB_GOAL}g</span></div>
//           <div className="macro-target-row"><span style={{ color: "#F6AD55" }}>⊙ Fat</span><span>{fat}g / {FAT_GOAL}g</span></div>
//           <div className="macro-target-row"><span style={{ color: "#68D391" }}>⊙ Fibre</span><span>{fibre}g / {FIBRE_GOAL}g</span></div>
//         </div>

//         <div className="macro-bar-row">
//           <div className="macro-bar" style={{ background: "#FC8181", width: `${Math.min(protein/PROTEIN_GOAL,1)*100}%` }} />
//           <div className="macro-bar" style={{ background: "#9F7AEA", width: `${Math.min(carbs/CARB_GOAL,1)*100}%` }} />
//           <div className="macro-bar" style={{ background: "#F6AD55", width: `${Math.min(fat/FAT_GOAL,1)*100}%` }} />
//           <div className="macro-bar" style={{ background: "#68D391", width: `${Math.min(fibre/FIBRE_GOAL,1)*100}%` }} />
//         </div>

//         <p className="split-section-label">ADD CALORIES</p>
//         <input className="eve-input" placeholder="Meal title" value={mealTitle} onChange={e => setMealTitle(e.target.value)} />
//         <input className="eve-input" placeholder="Calories" type="number" value={mealCals} onChange={e => setMealCals(e.target.value)} />
//         <button className="signupBtn" onClick={addMeal}>Add ↓</button>

//         {meals.map((m, i) => (
//           <div key={i} className="logged-meal">
//             <span>{m.title}</span>
//             <span style={{ color: "#63B3ED" }}>{m.calories} cal</span>
//           </div>
//         ))}
//       </div>

//       <div className="right">
//         <p className="split-section-label">MEAL PLAN</p>
//         {Object.entries(suggestions).map(([meal, items]) => (
//           <div key={meal} className="meal-section">
//             <p className="meal-type-label">· {meal}</p>
//             {items.map((item, i) => (
//               <div key={i} className="meal-card">
//                 <div className="meal-card-top">
//                   <div>
//                     <p className="meal-name">{item.name}</p>
//                     <p className="meal-meta">{item.calories} cal · {item.protein}g protein · {item.carbs}g carbs · {item.fat}g fat</p>
//                   </div>
//                   <button className="expand-btn" onClick={() => setExpanded(expanded === `${meal}-${i}` ? null : `${meal}-${i}`)}>
//                     {expanded === `${meal}-${i}` ? "▲" : "▼"}
//                   </button>
//                 </div>
//                 {expanded === `${meal}-${i}` && (
